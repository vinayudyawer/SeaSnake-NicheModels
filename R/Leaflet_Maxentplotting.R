...
maxentmap<-
  ...
  addCircleMarkers(lat=ap$lat, lng=ap$lon, radius= rad, weight=wt, opacity=op, color=1, group="A. apraefrontalis", fillOpacity=op, popup=paste("source =",ap$lab)) %>%
  addRasterImage(r[[2]], colors = p2, group="A. foliosquama", opacity = 0.7) %>%
  addCircleMarkers(lat=fo$lat, lng=fo$lon, radius= rad, weight=wt, opacity=op, color=1, group="A. foliosquama", fillOpacity = op, popup=paste("source =",fo$lab)) %>%
  addRasterImage(r[[3]], colors = p3, group="A. fuscus",  opacity = 0.7) %>%
  addCircleMarkers(lat=fu$lat, lng=fu$lon, radius= rad, weight=wt, opacity=op, color=1, group="A. fuscus", fillOpacity = op, popup=paste("source =",fu$lab)) %>%
  addRasterImage(r[[4]], colors = p4, group="A. pooleorum", opacity = 0.7) %>%
  addCircleMarkers(lat=po$lat, lng=po$lon, radius= rad, weight=wt, opacity=op, color=1, group="A. pooleorum", fillOpacity = op, popup=paste("source =",po$lab)) %>%
  addRasterImage(r[[5]], colors = p5, group="A. tenuis", opacity = 0.7) %>%
  addCircleMarkers(lat=te$lat, lng=te$lon, radius= rad, weight=wt, opacity=op, color=1, group="A. tenuis", fillOpacity = op, popup=paste("source =",te$lab)) %>%
